...
function getForecastItem(forecast, previousDate, previousTimePrefix, timezone){
  var datetime_cn = datetimeInChinese(forecast.time, timezone)
  var temp_cn = getTemperature(forecast)
  if (temp_cn.length > 0 && timezone.indexOf("America") >= 0) {
...
function getForecastItemDaily(forecast, previousDate, previousTimePrefix, timezone){
  var datetime_cn = datetimeInChinese(forecast.time, timezone)
...
function renderJson(data, cityIndex, handleResult) {
  ...
  function pushForecast(forecast) {
    forecastItem = getForecastItem(forecast, previousDate, previousTimePrefix, timezone)
  ...
  function pushDailyForecast(forecast) {
    forecastItem = getForecastItemDaily(forecast, previousDate, previousTimePrefix, timezone)
  ...
  handleResult("", {forecasts:forecasts, daily:dailyForecasts, timezone: timezone})
}
