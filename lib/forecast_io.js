...
function renderJson(data, cityIndex, res, didGetForecasts) {
  var forecasts = []
  var dailyForecasts = []
  var previousDate = "";
  ...
  didGetForecasts(forecasts, dailyForecasts, cityIndex, res, "")
}
function router_get_forecast(cityIndex, req, res, didGetForecasts) {
  ...
  request({
    ...
    if (!error && response.statusCode === 200) {
      renderJson(body, cityIndex, res, didGetForecasts)
    } else {
      error = error + response.statusCode
      didGetForecasts("", "", "", "", error)
    }
  })
}
